    ...
    <!--Begin 所在收货地区 Begin-->
        ...
        <span class="fr">
          <c:if test="${sessionScope.loginUser==null}">
            <span class="fl">你好，请<a href="${ctx}/pre/login.jsp"  style="color:#ff4e00;">登录</a>&nbsp;<a href="${ctx}/pre/register.jsp" style="color:#ff4e00;">免费注册</a>&nbsp;&nbsp;</span>
          </c:if>
          <c:if test="${sessionScope.loginUser!=null}">
            <span class="fl"><a href="${ctx}/user?action=index">${sessionScope.loginUser.u_userName}</a>&nbsp;|&nbsp;<a href="${ctx}/order?action=index&userId=${sessionScope.loginUser.u_id}">我的订单</a>&nbsp;</span>
          </c:if>
           <c:if test="${sessionScope.loginUser!=null && sessionScope.loginUser.u_type==2}">
<%--            <span class="fl">|&nbsp;<a href="${ctx}/admin/product?action=index&userId=${sessionScope.loginUser.u_id}">后台管理</a>&nbsp;</span>--%>
          </c:if>
           <c:if test="${sessionScope.loginUser!=null}">
             <span class="fl">|&nbsp;<a href="${ctx}/user?action=logOut" >注销</a></span>
          </c:if>
        </span>
  </div>
</div>
